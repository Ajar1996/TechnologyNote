...
RDB 可能会导致一定时间内的数据丢失，而 AOF 由于文件较大则会影响 Redis 的启动速度



4.0混合持久化原理

混合持久化是结合了 RDB 和 AOF 的优点，在写入的时候，先把当前的数据以 RDB 的形式写入文件的开头，再将后续的操作命令以 AOF 的格式存入文件，这样既能保证Redis 重启时的速度，又能减低数据丢失的风险。
...
![image-20220706150518927](../images/c-3Redis/image-20220706150518927.png)

优点：读写分离，提高效率，数据备份，提供多个副本

缺点：不具备自动容错和恢复功能，主节点故障，需要人工将从节点升为主节点
...
哨兵模式核心还是主从复制，只不过在相对于主从模式在主节点宕机导致不可写的情况下，多了一个竞选机制：从所有的从节点竞选出新的主节点。

![image-20220706150840689](../images/c-3Redis/image-20220706150840689.png)
...
优点：基于主从模式的，解决可主从模式中master故障不可以自动切换故障的问题。

缺点：

- 始终只有一个Redis主机来接收和处理写请求，写操作容易成为瓶颈
- 所有节点保存的都是全量数据，浪费内存空间，没有实现真正的分布式存储。
- 主结点宕机，选出先主机前，服务处于不可用状态。



#### 数据分片

通过Redis客户端预先定义好的路由规则(使用一致性哈希)，把对Key的访问转发到不同的Redis实例中，查询数据时把返回结果汇集。

![image-20220706152019982](../images/c-3Redis/image-20220706152019982.png)

#### 多主多从模式



![image-20220706152318842](../images/c-3Redis/image-20220706152318842.png)
