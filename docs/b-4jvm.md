...
![image-20220301140836612](../images/b-4jvm/image-20220301140836612.png)

- 加载：将类的Class文件加载到内存中，并为其生成Java.lang.class对象
- 验证：确保Java类符合JVM规范，保证安全性
- 准备：为类变量分配内存和初始值
- 解析：JVM常量池中的符号引用变为直接引用
- 初始化：对类进行初始化
...
2. ExtendsionClassLoader（拓展类加载器）：主要负责加载`%JAVA_HOME%/lib/ext`目录下的jar包和类
...
否则才会尝试加载。加载的时候，首先会把该请求委派该父类加载器的 `loadClass()` 处理，因此所有的请求最终都应该传送到顶层的启动类加载器 `BootstrapClassLoader` 中。当父类加载器无法处理时，才由自己来处理。

![image-20220301151205629](../images/b-4jvm/image-20220301151205629.png)
...
2. 分配内存：类加载检查通过后，就会在堆中为对象分配内存。分配方式有：指针碰撞和空闲列表。由堆是否规整决定的。
...
Java 堆还可以细分为：新生代和老年代：再细致一点有：Eden 空间、From Survivor、To Survivor 



![image-20220301165635050](../images/b-4jvm/image-20220301165635050.png)
...
- 对象优先在`eden`区进行分配
  - 经过第一次`Minor GC`仍然存货，并且能被`Survivor` 容纳的话，将被移动到` Survivor` 空间中
- 大对象直接进入老年代
- 长期存活的对象进入老年代（默认为大于 15 岁）
...
![image-20220704095029910](../images/b-4jvm/image-20220704095029910.png)

连线表示垃圾收集器可以配合使用。



![image-20220704102133280](../images/b-4jvm/image-20220704102133280.png)

`Serial`：单线程，串行的

`Serial Old`：Serial的老年代版本

![image-20220704102143281](../images/b-4jvm/image-20220704102143281.png)
...
`Parallel Scavenge`：多线程，注重吞吐量（CPU运行用户程序的时间占总时间的比值），别的收集器的目标是尽可能缩短垃圾收集时用户停顿的时间

`Parallel Old`：Parallel Scavenge的老年代版本

![image-20220704102251950](../images/b-4jvm/image-20220704102251950.png)

`CMS`：CMS指Concurrent Mark Sweep，Mark Sweep指的是标记-清除算法

分为四个阶段：

- 初始标记：仅仅标记GC Roots能关联到的对象，速度很快，需要stw
- 并发标记：进行GC Root追踪的过程，在回收过程中耗时时间最长，不需要停顿
- 重新标记：修正并发标记期间因用户程序继续运作而导致标记产生变动的那一部分的对象的标记记录，需要停顿。
- 并发清除：不需要停顿



G1：G1 收集器可以直接对新生代和老年代一起回收。
